' ======================================================================
' Script Aurea/Black List para hMailServer (v3.8 CORRIGIDO)
'
' CORREÇÃO v3.8: Wildcard logic CORRIGIDA (baseado em v3.7)
' - Política: AUTH > BLACKLIST > WHITELIST > DEFAULT
' - Suporte a wildcards (* e ?) CORRIGIDO
' - Integração correta com hMailServer via Result.Value / Result.Message
' - Otimizado para performance
' - Cache com reload automático (5 minutos)
'
' VERSÃO: 3.8 CORRIGIDO
' DATA: 19/11/2025
...
Sub CheckAndLoadCache()
    ...
    Else
        ...
        If diffMinutes >= CACHE_RELOAD_MINUTES Or diffMinutes < 0 Then
            needReload = True
        End If
    End If
...
Function LoadListFile(filename)
    ...
    If Not fso.FileExists(fullPath) Then
        WriteAuditLog "CACHE_WARNING: File not found: " & fullPath
        LoadListFile = Array()
        Exit Function
    End If
    ...
    If count > 0 Then
        WriteAuditLog "CACHE_LOAD: " & filename & " loaded (" & count & " entries)"
        LoadListFile = arr
    Else
        LoadListFile = Array()
    End If
...
' ========================================================
' FUNCAO DE MATCH COM WILDCARD - CORRIGIDA v3.8!
'
' CORREÇÃO CRÍTICA:
' - Substitui wildcards por placeholders ANTES de escapar
' - Escapa caracteres especiais de regex
' - Restaura wildcards como regex
'
' BUG ANTERIOR (v3.8 original):
