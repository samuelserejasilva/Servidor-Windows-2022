' ========================================================================
' EventHandlers.vbs - VERSÃO 3.7 DEBUG
' hMailServer Event Handlers with Aurea Black/White Lists
'
' VERSÃO DEBUG: Logs detalhados para diagnóstico
...
Const DEBUG_MODE = True  ' ← DEBUG ATIVADO! (para diagnóstico)
...
Sub OnSMTPData(oClient, oMessage)
   ...
   If DEBUG_MODE Then
      WriteAuditLog "DEBUG [EMAIL_RECEIVED]: FROM=" & senderEmail & " | DOMAIN=" & senderDomain & " | IP=" & clientIP
   End If
   ...
   If IsInListDebug(CACHE_WHITELIST_EMAILS, senderEmail, "WL_EMAIL") Then
   ...
   If IsInListDebug(CACHE_WHITELIST_DOMAINS, senderDomain, "WL_DOMAIN") Then
   ...
   If IsInListDebug(CACHE_WHITELIST_IPS, clientIP, "WL_IP") Then
   ...
   If IsInListDebug(CACHE_BLACKLIST_EMAILS, senderEmail, "BL_EMAIL") Then
   ...
   If IsInListDebug(CACHE_BLACKLIST_DOMAINS, senderDomain, "BL_DOMAIN") Then
   ...
   If IsInListDebug(CACHE_BLACKLIST_IPS, clientIP, "BL_IP") Then
...
' ========================================================================
' FUNÇÃO: IsInListDebug - Verifica se chave está na lista (COM DEBUG!)
'
' VERSÃO DEBUG: Logs detalhados mostrando:
'   - Qual lista está sendo verificada
'   - Quantas entradas estão sendo checadas
'   - Qual entrada deu match (se houver)
'   - Se o match foi exato ou wildcard
'
' CORREÇÃO v3.7: Lógica de wildcard CORRIGIDA!
...
'   - debugContext: Contexto para logs DEBUG (ex: "WL_EMAIL", "BL_DOMAIN")
'
' RETORNO:
...
Function IsInListDebug(Byval listCacheName, Byval key, Byval debugContext)
   ...
   If IsEmpty(key) Or Trim(key) = "" Then
      If DEBUG_MODE Then
         WriteAuditLog "DEBUG [" & debugContext & "]: Key is EMPTY, skipping check"
      End If
      Exit Function
   End If
   ...
   If IsEmpty(listData) Or Trim(listData) = "" Then
      If DEBUG_MODE Then
         WriteAuditLog "DEBUG [" & debugContext & "]: Cache '" & listCacheName & "' is EMPTY"
      End If
      Exit Function
   End If
   ...
   ' LOG DEBUG: Início da verificação
   If DEBUG_MODE Then
      WriteAuditLog "DEBUG [" & debugContext & "]: Checking key='" & key & "' against " & (ub-lb+1) & " entries in '" & listCacheName & "'"
   End If
   ...
   For i = lb To ub
      ...
      ElseIf InStr(item, "*") > 0 Or InStr(item, "?") > 0 Then
         ...
         If regex.Test(key) Then
            If DEBUG_MODE Then
               WriteAuditLog "DEBUG [" & debugContext & "]: ✅ MATCH! Wildcard '" & item & "' (regex: " & pattern & ") matched '" & key & "'"
            End If
            IsInListDebug = True
            Exit Function
         End If
      ...
      Else
         ...
         If LCase(item) = LCase(key) Then
            If DEBUG_MODE Then
               WriteAuditLog "DEBUG [" & debugContext & "]: ✅ MATCH! Exact match '" & item & "' == '" & key & "'"
            End If
            IsInListDebug = True
            Exit Function
         End If
   ...
   ' Não encontrou match
   If DEBUG_MODE Then
      WriteAuditLog "DEBUG [" & debugContext & "]: ❌ NO MATCH for '" & key & "' (" & (ub-lb+1) & " entries checked)"
   End If
